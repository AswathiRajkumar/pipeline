    ...
    stages {
        ...
        stage('Deploy to AWS Ubuntu Server') {
            steps {
                ...
                sh '''
                scp -o StrictHostKeyChecking=no -r ./ ubuntu@http://16.170.240.213:/var/www/myapp
                ssh ubuntu@http://16.170.240.213 'pm2 restart myapp || pm2 start /var/www/myapp/index.js --name myapp'
                '''
            }
        }
    }
